...
int checkerboard[checkerboard_x][checkerboard_y], checkerboard_history[checkerboard_x][checkerboard_y][512];//棋盤 歷史棋盤(空2 黑0 白1 )
...
void show()//顯示棋盤
{
    ...
    for (int i = 0; i < checkerboard_x; ++i) {//備份棋盤
        for (int j = 0; j < checkerboard_y; ++j) {
            checkerboard_history[i][j][history_num] = checkerboard[i][j];
        }
    }
    history_num++;
            ...
            if (((letter >= 'a' && letter <= 'o') || (letter >= 'A' && letter <= 'O')))//限制大小寫A到O
            {
                ...
                if (number > 0 && number <= 16)//限制數字
                {
                    ...
                    else if ((letter_num + 1) == 15 && number == 16)//o16,悔棋
                    {
                        if (history_flag >= 2)
                        {
                            if (history_num > 1)
                            {
                                ...
                                show();
                                if (language == 0) {
                                    printf("ERROR1(NO history checkerboard)\n");
                                    printf("AGAIN\n");
                                }
                                else if (language == 1)
                                {
                    ...
                    else if (checkerboard[letter_num + 1][number] == 2)//如果當前輸入位置為空可下棋
                    {
                        ...
                        show();
                        if (language == 0) {
                            printf("ERROR3(Enter a duplicate location)\n");
                            printf("AGAIN\n");
                        }
                        else if (language == 1)
                        {
                            printf("錯誤3(輸入重複位置)\in");
                            printf("再次輸入\n");
                        }
                        printf("\n");
                        input(1);
                    ...
                    show();
                    if (language == 0) {
                        printf("ERROR4(Number is too big)\n");
                        printf("AGAIN\n");
                    }
                    else if (language == 1)
                    {
                ...
                show();
                if (language == 0) {
                    printf("ERROR5(Does not meet input requirements)\n");
                    printf("AGAIN\n");
                }
                else if (language == 1)
                {
...
int main(void)
{
    ...
    printf("The black stones are placed first, and they are placed at empty spots on the board in turn.\n\n");
    ...
    for (short int y = 1; y <= checkerboard_y; y++) {// 初始設置全為空
        for (short int x = 1; x <= checkerboard_x; x++) {
            for (short int z = 1; z <= 512; z++) {
                checkerboard_history[x][y][z] = 2; 
            }
        }
    }
